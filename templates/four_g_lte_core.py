...
class FourGLTECore(ServiceProfileTemplate):
    ...
    def __init__(self, name: str, domain_name: str, bandwidth: int):
        ...
        self.spgw_c = SPGWTemplate(self.name)
        self.spgw_u = SPGWTemplate(self.name)
        self.__build()
    ...
    def __build(self):
        ...
        spgw_c = VMRequirement(2, self.spgw_c.vm_name, self.nova.get_flavor_by_id(self.spgw_c.flavor),
                               image_id=self.spgw_c.image_id, network_id=self.spgw_c.network_id,
                               ip_address=self.spgw_c.ip_address)
